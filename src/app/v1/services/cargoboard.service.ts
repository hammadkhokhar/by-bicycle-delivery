...
class CargoboardServices {
  ...
  async getDistance(orderRequest: IOrder): Promise<number> {
    try {
      const distanceRequestConfig = this.createDistanceRequestConfig(orderRequest);
      const response:number = (await this.makeDistanceRequest(distanceRequestConfig)).data.distance;
      return response;
    } catch (error) {
  ...
  private createDistanceRequestConfig(orderRequest: IOrder): AxiosRequestConfig {
  ...
  private async makeDistanceRequest(config: AxiosRequestConfig): Promise<AxiosResponse> {
    try {
