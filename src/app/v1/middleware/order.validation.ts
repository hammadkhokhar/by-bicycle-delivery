...
import { sendErrorResponse } from '../utils/api-errors.util'
/**
 * Middleware for validating incoming requests.
 */
class ValidationMiddleware {
  /**
   * Validates the initial quote request.
  ...
  validateOrder() {
    return async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (payloadSizeInBytes > 300) {
        sendErrorResponse(res, 422, 'Request payload size is too large.', {})
        return
      }
      try {
  ...
  /**
   * Validates the body of the booking request.
   */
  validateBooking() {
    return async (req: Request, res: Response, next: NextFunction) => {
      const payloadSizeInBytes = Buffer.from(JSON.stringify(req.body)).length
      // Check if the payload size exceeds a certain limit
      if (payloadSizeInBytes > 130) {
        sendErrorResponse(res, 422, 'Request payload size is too large.', {})
        return
      }
      try {
        const uuidSchema = z.string().uuid()
        const uuidValidation = await uuidSchema.safeParseAsync(
          req.params.quoteId,
        )
        if (!uuidValidation.success) {
          sendErrorResponse(res, 422, 'Invalid quote id', {})
          return
        }
        ...
        const validatedData = await bookingSchema.parseAsync(req.body)
        req.body = validatedData
        next()
      } catch (error) {
        if (error instanceof ZodError) {
